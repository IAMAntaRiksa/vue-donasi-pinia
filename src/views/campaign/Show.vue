        ...
        <div class="container mx-auto grid grid-cols-1 p-3 sm:w-full md:w-5/12">
            ...
            <div class="bg-white rounded-md shadow-md p-3">
                ...
                <div v-if="maxDate(campaign.max_date) == true">
                    <div class="mt-5">
                        <button
                            class="bg-yellow-500 py-3 rounded-md shadow-md text-xl w-full uppercase font-bold focus:outline-none opacity-50 cursor-not-allowed">Donasi
                            Ditutup!</button>
                    </div>
                </div>
                <div v-else>
...
export default {
    setup() {
        ...
        onMounted(() => {
            store.getDetilCampaignData(route.params.slug)
        })

        const campaign = computed(() => {
            return store.detailCampaignGetter
        })

        const sumDonation = computed(() => {
            return store.subDonation
        })

        const donations = computed(() => {
            return store.donationsGetter
        })
        const user = computed(() => {
            return store.user
        })
